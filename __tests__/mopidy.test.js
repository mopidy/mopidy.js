...
describe("constructor", () => {
    test("connects when autoConnect is true", () => {
        ...
        expect(Mopidy.WebSocket)
            .toHaveBeenCalledWith("ws://" + currentHost + "/mopidy/ws");
    });
...
describe("._reconnect", () => {
    ...
    test("tries to connect after an increasing backoff delay", () => {
        ...
        const connectStub = jest.spyOn(this.mopidy, "connect")
            .mockImplementation(() => { });
...
describe("._resetBackoffDelay", () => {
    ...
    test("resets the backoff delay to the minimum value", () => {
        ...
        expect(this.mopidy._backoffDelay)
            .toBe(this.mopidy._settings.backoffDelayMin);
    });
});
...
describe("._handleMessage", () => {
    test("is called on 'websocket:incomingMessage' event", () => {
        const messageEvent = {};
        const stub = jest.spyOn(this.mopidy, "_handleMessage")
            .mockImplementation(() => { });
        this.mopidy._delegateEvents();
    ...
    test("passes events on to _handleEvent", () => {
        const stub = jest.spyOn(this.mopidy, "_handleEvent")
            .mockImplementation(() => { });
        const message = {
        ...
        promise
            .catch((error) => {
                ...
                expect(error.message)
                    .toBe("Response without 'result' or 'error' received");
...
describe("._handleEvent", () => {
    test("emits server side even on Mopidy object", () => {
        ...
        expect(spy)
            .toBeCalledWith("event:trackPlaybackStarted", { track: track });
    });
});
...
describe("._getApiSpec", () => {
    ...
    test("gets API description from server and calls _createApi", done => {
        const methods = {};
        const sendStub = jest.spyOn(this.mopidy, "_send")
            .mockReturnValue(when.resolve(methods));
        const createApiStub = jest.spyOn(this.mopidy, "_createApi")
            .mockImplementation(() => { });
...
describe("._createApi", () => {
    ...
    describe("by-position-only calling convention", () => {
        beforeEach(() => {
            ...
            this.sendStub = jest.spyOn(this.mopidy, "_send")
                .mockImplementation(() => { });
        });
    ...
    describe("by-position-or-by-name calling convention", () => {
        ...
        test("must be turned on manually", () => {
            expect(this.mopidy._settings.callingConvention)
                .toBe("by-position-or-by-name");
        });
